...
- 🔋 **电池容量修正**：优先动态查找并 bind-mount 修补实际 `power_profile.xml`（仅替换 `battery.capacity`），并保留 RRO 作为备用
...
**重要**：若未设置 `battery_capacity`，模块不会打包/启用任何电池覆盖功能，系统将保持原厂容量（符合“除非被要求，否則不更改硬體資訊”的原则）。
...
4. 若 ROM 允许，则系统会加载 overlay；否则会被忽略（因此作为备用）

> `battery-overlay.apk` 仅在设置了 `battery_capacity` 时才会被打包；无配置构建不会携带默认容量。
...
│   ├── battery_capacity.conf      # 电池容量配置（可选，触发 bind-mount 修补）
│   └── META-INF/
│       └── com/google/android/
